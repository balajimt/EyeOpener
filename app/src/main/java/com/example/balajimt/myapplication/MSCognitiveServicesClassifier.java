...
public class MSCognitiveServicesClassifier {
    ...
    public MSCognitiveServicesClassifier(final Context context,int flagFile) {
		flag = flagFile;
		if(flagFile==1)//qr
			inferenceInterface = new TensorFlowInferenceInterface(context.getAssets(), "file:///android_asset/qr.pb");
		else if(flagFile==2)//money
			inferenceInterface = new TensorFlowInferenceInterface(context.getAssets(), "file:///android_asset/currency.pb");
		else if(flagFile==3)//coin
			inferenceInterface = new TensorFlowInferenceInterface(context.getAssets(), "file:///android_asset/coin.pb");
        loadLabels(context);
    }
    ...
    private void loadLabels(final Context context) {
        ...
        try {
			if(flag==1)//qr
				...
				temp="coin.txt";
            final InputStream inputStream = assetManager.open(temp);
